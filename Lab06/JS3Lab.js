...
//two seperate 
console.log(alphabetArray.reverse()); //NOTE: reverse changes the original array 
...
console.log('Step 3: ', array1[5]); //use this ONLY if you KNOW how many elements are in an array 
//most times you dont know the length of the array so use this: 

console.log(array1[array1.length-1])
//this is how you target the last element to find the last element 

console.log('Step 4: ', array1.push(16, 3)); //prints 8
console.log(array1); //prints the new array with 16 and 3 
 ...
 * ↓ YOUR CODE HERE ↓ */
let myTodoList = [];
myTodoList.push('Clean Room', 'Wash Dishes', 'Do Homework');
...
let yourTodoList = ['pay bills', 'groceries'];
console.log('Step 4: ', yourTodoList);

console.log('Step 5: '); //two ways you can do this 
//let ourTodoList = myTodoList.concat(yourTodoList);
//console.log(ourTodoList); //Concat route 

let ourTodoList = [...myTodoList, ...yourTodoList];
console.log(ourTodoList); //spread route 
...
function reverse(x){
       if(typeof x === 'boolean'){
              ...
              return x.toString().split('').reverse('').join(''); //we can't reverse a number so we have to convert it into a string, split it up, reverse, it then put it back together 
       } else if (typeof x === 'string'){
              return x.split('').reverse('').join(''); //doing the same thing as the number but this is already a string so leave out x.toString()       
